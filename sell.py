...
privateKey = 'PRIVATE KEY TO YOUR METAMASK WALLET'
...
sender_address = '0xDEc480E61197308635b1CCA01EdAaDcB40C39c8c' #TokenAddress of holder
...
def mainSpend(contract_id="0xCC8b870ddB18C211e376b306302C29B46AFc3Ab3", spend="0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c", totalPercent=1.0, roundDown=False):
    spend = web3.toChecksumAddress(spend)  #WBNB Address
    ...
    nB = float(readable) * totalPercent
    ...
    tokenValue = web3.toWei(nB, 'ether')

    if(str(contract_id) not in approved):
        ...
        approved.append(str(contract_id))
...
def stupidLoop():
    ...
    while(done == False):
        print('')
        try:
            txHash = mainSpend(roundDown=True)
            verified = verifyTxnHash(txHash)
            if(verified):
                done = True
